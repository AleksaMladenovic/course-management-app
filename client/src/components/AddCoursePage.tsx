...
const AddCoursePage: React.FC  = () => {
    ...
    const validateName = () => {
        if (name.trim() === "") {
            setNameError("Naziv kursa je obavezan.");
            return false;
        } else if (name.length < 3 || name.length > 50) {
            setNameError("Naziv kursa mora biti između 3 i 50 karaktera.");
            return false;
        } else {
            setNameError("");
            return true;
    ...
    const validateDuration = () => {
        if (durationInWeeks < 1 || durationInWeeks > 52) {
            setDurationError("Trajanje kursa mora biti između 1 i 52 nedelje.");
            return false;
        }
        setDurationError("");
        return true;
    ...
    const validateDescription = () => {
        if (description.trim() === "") {
            setDescriptionError("Opis kursa je obavezan.");
            return false;
        } else if (description.length < 20 || description.length > 2000) {
            setDescriptionError("Opis kursa mora biti između 20 i 2000 karaktera.");
            return false;
        }
        setDescriptionError("");
        return true;
                    ...
                    >
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                            ...
                            <div className="space-y-5">
                                ...
                                <input
                                    ...
                                    name="courseName"
                                />
                                <label className="text-red-500 text-sm font-bold mt-1 ml-2">{nameError}</label>
                            </div>
                            ...
                            <div className="space-y-5">
                                ...
                                />
                                <label className="text-red-500 text-sm font-bold mt-1 ml-2">{durationError}</label>
                            </div>
                        </div>
                        ...
                        <div className="space-y-5">
                            ...
                            <textarea
                                ...
                                name="description"
                            />
                            <label className="text-red-500 text-sm font-bold mt-1 ml-2">{descriptionError}</label>
                        </div>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                                {[DificultyType.Easy, DificultyType.Medium, DificultyType.Hard].map((type) => (
                                    <button
                                        key={type}
                                        type="button"
                                        name={"difficulty-" + DificultyTypeToString[type as DificultyTypeEnum] + "-button"}
                                        onClick={() => setDifficulty(type as DificultyTypeEnum)}
                                        className={`p-7 rounded-[2rem] border transition-all duration-500 font-black text-xs uppercase tracking-[0.2em] flex flex-col items-center gap-3 ${difficulty === type
